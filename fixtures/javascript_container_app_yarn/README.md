...
Run `yarn install --immutable`, `yarn run lint`, `yarn test`, and `yarn run build` in this directory. Tests render the React component and write LCOV coverage. The Docker smoke test verifies the served page includes `FIXTURE_MESSAGE_SUFFIX=runtime-check` and that the client bundle is available.
